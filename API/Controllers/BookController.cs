...
public class BookController : BaseApiController
{
    ...
    public async Task<ActionResult<BookInfoDto>> GetBookInfo(int chapterId)
    {
        ...
        switch (dto.SeriesFormat)
        {
            case MangaFormat.Epub:
            {
                ...
                var file = _cacheService.GetCachedFile(chapterId, mangaFile.FileMetadata.Path);
    ...
    public async Task<ActionResult> GetBookPageResources(int chapterId, [FromQuery] string file)
    {
        ...
        var cachedFilePath = Path.Join(_cacheService.GetCachePath(chapterId), Path.GetFileName(chapter.Files.ElementAt(0).FileMetadata.Path));
