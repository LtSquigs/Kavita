...
import {
  allFileTypeGroup,
  FileTypeGroup,
} from '../../../_models/library/file-type-group.enum';
...
export class LibrarySettingsModalComponent implements OnInit {
  ...
  protected readonly excludePatternTooltip =
    `<span>` +
    translate('library-settings-modal.exclude-patterns-tooltip') +
    `<a class="ms-1" href="${
      WikiLink.ScannerExclude
    }" rel="noopener noreferrer" target="_blank">${translate(
      'library-settings-modal.help'
    )}` +
  ...
  libraryForm: FormGroup = new FormGroup({
    ...
    type: new FormControl<LibraryType>(LibraryType.Manga, {
      nonNullable: true,
      validators: [Validators.required],
    }),
    folderWatching: new FormControl<boolean>(true, {
      nonNullable: true,
      validators: [Validators.required],
    }),
    includeInDashboard: new FormControl<boolean>(true, {
      nonNullable: true,
      validators: [Validators.required],
    }),
    includeInRecommended: new FormControl<boolean>(true, {
      nonNullable: true,
      validators: [Validators.required],
    }),
    includeInSearch: new FormControl<boolean>(true, {
      nonNullable: true,
      validators: [Validators.required],
    }),
    extractChaptersFromVolumes: new FormControl<boolean>(false, {
      nonNullable: true,
      validators: [Validators.required],
    }),
    manageCollections: new FormControl<boolean>(true, {
      nonNullable: true,
      validators: [Validators.required],
    }),
    manageReadingLists: new FormControl<boolean>(true, {
      nonNullable: true,
      validators: [Validators.required],
    }),
    allowScrobbling: new FormControl<boolean>(true, {
      nonNullable: true,
      validators: [Validators.required],
    }),
    collapseSeriesRelationships: new FormControl<boolean>(false, {
      nonNullable: true,
      validators: [Validators.required],
    }),
  });
  ...
  get IsKavitaPlusEligible() {
    const libType = parseInt(
      this.libraryForm.get('type')?.value + '',
      10
    ) as LibraryType;
  ...
  ngOnInit(): void {
    ...
    if (this.library?.coverImage != null && this.library?.coverImage !== '') {
      this.imageUrls.push(
        this.imageService.getLibraryCoverImage(this.library.id)
      );
      this.cdRef.markForCheck();
    }
    ...
    if (
      this.library &&
      !(
        this.library.type === LibraryType.Manga ||
        this.library.type === LibraryType.LightNovel
      )
    ) {
      ...
      ?.valueChanges.pipe(
        ...
        tap((exists) => {
          const isExistingName =
            this.libraryForm.get('name')?.value === this.library?.name;
    ...
    this.libraryForm
      ...
      ?.valueChanges.pipe(
        tap((type: LibraryType) => {
          ...
          this.libraryForm
            .get('allowScrobbling')
            ?.setValue(this.IsKavitaPlusEligible);
  ...
  setValues() {
    if (this.library !== undefined) {
      ...
      this.libraryForm
        .get('folderWatching')
        ?.setValue(this.library.folderWatching);
      this.libraryForm
        .get('includeInDashboard')
        ?.setValue(this.library.includeInDashboard);
      this.libraryForm
        .get('includeInRecommended')
        ?.setValue(this.library.includeInRecommended);
      this.libraryForm
        .get('includeInSearch')
        ?.setValue(this.library.includeInSearch);
      this.libraryForm
        .get('extractChaptersFromVolumes')
        ?.setValue(this.library.extractChaptersFromVolumes);
      this.libraryForm
        .get('manageCollections')
        ?.setValue(this.library.manageCollections);
      this.libraryForm
        .get('manageReadingLists')
        ?.setValue(this.library.manageReadingLists);
      this.libraryForm
        .get('collapseSeriesRelationships')
        ?.setValue(this.library.collapseSeriesRelationships);
      this.libraryForm
        .get('allowScrobbling')
        ?.setValue(this.library.allowScrobbling);
      this.selectedFolders = this.library.folders;
      this.madeChanges = false;
      for (let fileTypeGroup of allFileTypeGroup) {
        this.libraryForm.addControl(
          fileTypeGroup + '',
          new FormControl(
            this.library.libraryFileTypes.includes(fileTypeGroup),
            []
          )
        );
      }
  ...
  forceScan() {
    this.libraryService.scan(this.library!.id, true).subscribe(() => {
      this.toastr.info(
        translate('toasts.forced-scan-queued', { name: this.library!.name })
      );
      this.close();
    });
  }
  ...
  async save() {
    ...
    if (this.library !== undefined) {
      ...
      model.folders = model.folders.map((item: string) =>
        item.startsWith('\\') ? item.substr(1, item.length) : item
      );
      ...
      if (model.type !== this.library.type) {
        if (
          !(await this.confirmService.confirm(
            translate('toasts.confirm-library-type-change')
          ))
        )
          return;
      }
  ...
  applyCoverImage(coverUrl: string) {
    this.uploadService
      .updateLibraryCoverImage(this.library!.id, coverUrl)
      .subscribe(() => {});
  }
  ...
  resetCoverImage() {
    this.uploadService
      .updateLibraryCoverImage(this.library!.id, '', false)
      .subscribe(() => {});
  }
  ...
  isNextDisabled() {
    switch (this.setupStep) {
      case StepID.General:
        return (
          this.libraryForm.get('name')?.invalid ||
          this.libraryForm.get('type')?.invalid
        );
      case StepID.Folder:
  ...
  getTasks() {
    ...
    return this.actionFactoryService.getActionablesForSettingsPage(
      this.actionFactoryService.getLibraryActions(this.runTask.bind(this)),
      blackList
    );
  }
  ...
  async runTask(action: ActionItem<Library>) {
    switch (action.action) {
      ...
      case Action.GenerateColorScape:
        await this.actionService.refreshLibraryMetadata(
          this.library!,
          undefined,
          false
        );
